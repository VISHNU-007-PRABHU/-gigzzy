...
class CancelRequest extends React.Component {
    ...
    handleTableChange =async pagination => {
        ...
        await client.query({
            query: GET_BOOKING,
            variables: { limit: pager.pageSize, page: pager.current, booking_status: [8,11],payment_status:[0] },
    ...
    fetch_booking = async() => {
        // option 2  === user,provider cancel for after booking
        this.setState({ loading: true });

        await client.query({
            query: GET_BOOKING,
            variables: { limit: this.state.pagination.pageSize, page: this.state.pagination.current, booking_status:[8,11],payment_status:[0] },
            fetchPolicy: 'no-cache',
        }).then(result => {
            const pagination = { ...this.state.pagination };
            pagination.total = result.data.get_booking.pageInfo.totalDocs;
...
export default Form.create()(CancelRequest);