...
class UserRequest extends React.Component {
    ...
    handleTableChange =async pagination => {
        ...
        await client.query({
            query: GET_BOOKING,
            variables: { limit: pager.pageSize, page: pager.current, booking_status: [12] },
    ...
    fetch_booking = async() => {
        ...
        await client.query({
            query: GET_BOOKING,
            variables: { limit: this.state.pagination.pageSize, page: this.state.pagination.current, booking_status:[12] },
            fetchPolicy: 'no-cache',
        }).then(result => {
            const pagination = { ...this.state.pagination };
            pagination.total = result.data.get_booking.pageInfo.totalDocs;
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (text, record) => {
                    ...
                    if(record.booking_status === 12){
                        return <span title="Status"> <Tag color="cyan">User Request</Tag> </span>;
                    }
                },
            },
...
export default Form.create()(UserRequest);